...
{
    // make_matrix(in.rows, l.w.cols);

    // TODO: 3.1 - run the network forward
    matrix out = matmul(in, l.w); // xw
    forward_bias(out, l.b); // xw + b
    activate_matrix(out, l.activation); //f(xw + b)
...
void backward_connected_layer(layer l, matrix prev_delta)
{
    ...
    // updates for our weights, which are stored in l.dw
     matrix x_t = transpose_matrix(in);
     matrix prod = matmul(x_t, delta);
     axpy_matrix(1.0, prod, l.dw);
     // l.dw = copy_matrix(prod);
    ...
    if(prev_delta.data){
        ...
        matrix tmp = matmul(delta, w_t);
        axpy_matrix(1.0, tmp, prev_delta);
